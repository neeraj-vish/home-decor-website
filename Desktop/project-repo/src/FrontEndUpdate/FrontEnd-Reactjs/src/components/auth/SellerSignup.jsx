...
const checkUniqueField = async (fieldName, value) => {
  const response = await fetch(
    `http://localhost:8083/api/sellers/exists?${fieldName}=${encodeURIComponent(
      value
    )}`
  );
...
const SellerSignup = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await fetch("http://localhost:8083/api/sellers/create", {
        method: "POST",
  ...
  className={`form-control mb-2 ${errors.companyName ? "is-invalid" : ""}`}
  {...register("companyName", {
    ...
    validate: async (value) =>
      !(await checkUniqueField("companyName", value)) || "Company Name already exists",
  })}
/>
{errors.companyName && (
